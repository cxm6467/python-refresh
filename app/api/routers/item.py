...
from app.api.dependencies import SessionDep, ItemServiceDep
...
async def get_item(id: int, session: SessionDep, service: ItemServiceDep) -> Item:
...
async def get_items(session: SessionDep, service: ItemServiceDep) -> dict[int, Item]:
    items = await service.get_all()
    return {item.id: item for item in items}


@router.post("/", response_model=Item, status_code=201)
async def create_item(item: ItemCreate, session: SessionDep, service: ItemServiceDep) -> Item:
